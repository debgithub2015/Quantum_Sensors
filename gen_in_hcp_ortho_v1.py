...
def create_POSCAR(atoms,lattice_scale,cellX,cellY,cellZ,str_name,cartesian):
    ...
    transform_matrix= array([[(2./sqrt(3.)), -(1./sqrt(3.)), (1./2.)], [ -(1./sqrt(3.)), (2./sqrt(3.)), (1./2.)], [-(1./sqrt(3.)), -(1./sqrt(3.)), (1./2.)]] )
      
    print(transform_matrix,transform_matrix.transpose())

# new cell generation

    new_cell=dot(transform_matrix.transpose(),old_cell)

    print(new_cell)

    P_matrix=linalg.inv(transform_matrix)
...
# new coordinate
   ...
   # coord_shift=coord-new_origin
   # 
   # coord_trans=[]
   # for i in coord_shift:
   #    trans_i=dot(P_matrix,i)
   #    coord_trans.append(trans_i)
   #
   # print(coord)
   # print(coord_trans)
